...
const mockMembers = [
  { id: 'm1', name: 'たなか', totalDurationSeconds: 3600, sessionIds: ['s1', 's2'] },
  { id: 'm2', name: 'すずき', totalDurationSeconds: 1800, sessionIds: ['s1'] },
  { id: 'm3', name: 'やまだ', totalDurationSeconds: 7200, sessionIds: ['s1', 's2', 's3'] },
...
describe('MemberList', () => {
  describe('名称昇順ソート', () => {
    it('メンバーが名称の昇順で表示されること', () => {
      ...
      // localeCompare('ja') による昇順: Alice → すずき → たなか → やまだ
      expect(names).toEqual(['Alice', 'すずき', 'たなか', 'やまだ']);
    });

    it('日本語名・英語名が混在しても正しくソートされること', () => {
      const mixedMembers = [
        { id: 'm1', name: 'Bob', totalDurationSeconds: 100, sessionIds: ['s1'] },
        { id: 'm2', name: 'あいだ', totalDurationSeconds: 200, sessionIds: ['s1'] },
  ...
  describe('インクリメンタルサーチ', () => {
    it('検索文字列を含むメンバーのみ表示されること（部分一致）', () => {
      ...
      const searchInput = screen.getByPlaceholderText('名前で検索...');
      fireEvent.change(searchInput, { target: { value: 'すずき' } });
    ...
    it('検索フィルタ適用後もソート順が維持されること', () => {
      const members = [
        { id: 'm1', name: 'たなか一郎', totalDurationSeconds: 100, sessionIds: ['s1'] },
        { id: 'm2', name: 'すずき', totalDurationSeconds: 200, sessionIds: ['s1'] },
        { id: 'm3', name: 'たなか二郎', totalDurationSeconds: 300, sessionIds: ['s1'] },
      ];
      renderMemberList(members);

      const searchInput = screen.getByPlaceholderText('名前で検索...');
      fireEvent.change(searchInput, { target: { value: 'たなか' } });

      const rows = screen.getAllByTestId('member-row');
      const names = rows.map((row) => row.querySelector('h3').textContent);

      // フィルタ後も名称昇順
      expect(names).toEqual(['たなか一郎', 'たなか二郎']);
    });
  });
});
