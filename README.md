...
- E2E テストは開発サーバー（`http://localhost:5173`）を自動起動してブラウザ操作を検証する
- `waitUntil: 'domcontentloaded'` に統一し、画面遷移後は主要 UI 要素の表示を明示的に待機することで安定性を確保している
- CI 環境ではワーカー数 1 でシーケンシャル実行し、ローカルでは並列実行で高速化する
- テスト失敗時はスクリーンショットとトレースが自動保存される（`tests/playwright-report/` 配下）
