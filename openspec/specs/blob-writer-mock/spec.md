...
開発環境（`import.meta.env.DEV === true`）かつダミートークン（`token === "dev"`）を使用している場合、BlobWriterは実際のAzure Blob Storageへのリクエストを送信しない。ローカルの`dev-fixtures/data/`ディレクトリに書き込まなければならない（MUST）。
