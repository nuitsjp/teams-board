...
現在のインフラスクリプト群（`scripts/infra/` 配下の5ファイル）は、Azure環境固有パラメータを `local.settings.json`（JSON形式）から読み込む共通ヘルパー `Load-LocalSettings.ps1` を使用している。一方、フロントエンド（Vite）は `.env` ファイルから `import.meta.env` 経由で環境変数を読み込んでいる。
