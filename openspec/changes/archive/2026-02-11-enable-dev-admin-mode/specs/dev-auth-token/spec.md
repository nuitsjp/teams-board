...
- **WHEN** 開発環境において`?token=dev`でアクセスする
...
- **WHEN** 開発環境において`?token=<valid-sas-token>`でアクセスする（`dev`以外のトークン）
...
- **WHEN** `pnpm run dev`にて起動した環境でコードを実行する
- **THEN** `import.meta.env.DEV`が`true`を返す
- **THEN** ダミートークン`dev`が有効になる

#### Scenario: 本番ビルドでの検出

- **WHEN** `pnpm run build`にてビルドされたコードを実行する
...
本番環境において`?token=dev`が管理者権限を付与してはならない（MUST NOT）。

#### Scenario: 本番環境での不正アクセス試行

- **WHEN** 本番環境（Azure Blob Storage静的サイト）において`?token=dev`でアクセスする
- **THEN** `useAuth`が`isAdmin: false`を返す
- **THEN** 管理者機能へのアクセスが拒否される
- **THEN** コンソールへエラーや警告が表示されない（攻撃者に情報を与えない）
