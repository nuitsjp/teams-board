...
現状、E2E補助コードはルート直下の `e2e-helpers/` に配置され、利用側の `e2e/` とは分離されています。実際の参照元は `e2e/` 配下（spec、global setup、global teardown）のみであり、テスト実装の責務境界とディレクトリ境界が一致していません。

この不一致により、次の課題が発生しています。
