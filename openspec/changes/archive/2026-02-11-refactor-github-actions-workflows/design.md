...
- **パスフィルタなし**: ドキュメントのみの変更でもフルパイプラインが実行される
...
- CI効率の向上: 重複実行の完全排除、適切なパスフィルタによる最適化
...
- コスト削減: 無駄なGitHub Actionsの実行を削減

**代替案**:
- パスフィルタなし: すべての変更でCIが実行され、リソース浪費
- 緩いフィルタ: ドキュメント変更も含めると無駄な実行が残る
...
**代替案**:
- feature branchの push で自動デプロイ: 無駄な実行が多く、制御が困難
...
**[リスク] パスフィルタが厳格すぎると、必要なビルドがスキップされる可能性**
- **軽減策**: 設定ファイル（package.json, pnpm-lock.yaml, vite.config.js, .eslintrc.cjs）もフィルタに含める
...
- **影響**: リソース使用量は変わらないが、両方が成功する必要がある
- **メリット**: CIとデプロイが独立しており、デプロイ失敗時もCI結果は保持される
