...
function serveDevFixtures() {
    const handler = async (req, res, next) => {
        ...
        // POSTリクエスト: 開発用削除処理
        if (req.method === 'POST' && req.url?.startsWith('/dev-fixtures-delete')) {
            console.log('[dev-fixtures-delete] リクエスト受信');
            let body = '';
            req.on('data', (chunk) => {
                body += chunk.toString();
            });
            req.on('end', async () => {
                try {
                    ...
                    console.log('[dev-fixtures-delete] パス:', path);
                    if (!path) {
                        res.statusCode = 400;
                        res.setHeader('Content-Type', 'application/json');
                        res.end(JSON.stringify({ success: false, error: 'pathが必要です' }));
                        return;
                    }
                    ...
                    console.log('[dev-fixtures-delete] ファイル削除成功:', filePath);

                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'application/json');
                    res.end(JSON.stringify({ success: true }));
                } catch (error) {
                    console.error('[dev-fixtures-delete] エラー:', error);
                    res.statusCode = 500;
                    res.setHeader('Content-Type', 'application/json');
                    res.end(
                        JSON.stringify({
