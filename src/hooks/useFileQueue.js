        ...
        case 'SELECT_GROUP': {
            ...
            return {
                ...
                queue: state.queue.map((item) => {
                    ...
                    return {
                        ...item,
                        groupOverride: { groupId, groupName },
                        status: 'ready',
                    };
                }),
            };
...
/**
 * ファイルキュー状態管理のカスタムHook（V2: 重複検出なし）
...
export function useFileQueue(csvTransformer) {
    ...
    const csvTransformerRef = useRef(csvTransformer);
    csvTransformerRef.current = csvTransformer;
    ...
    const parsePendingItem = useCallback(async (item) => {
        ...
        const result = await csvTransformerRef.current.parse(item.file);
        if (!result.ok) {
            dispatch({
                type: 'VALIDATE_ERROR',
                payload: { id: item.id, errors: result.errors },
            });
            return;
        }

        // グループ名が空の場合は missing_group ステータス
        if (!result.parsedSession.groupName) {
    ...
    const updateStatus = useCallback((fileId, status, extra) => {
        switch (status) {
            ...
            case 'save_failed':
                dispatch({
                    type: 'SAVE_FAIL',
                    payload: { id: fileId, errors: extra?.errors || [] },
                });
                break;
            case 'ready':
